...
int main()
{
    ...
    f = fopen("/home/user/conclusion.txt","w");
    fprintf(f, "=========================================================================\n");
    for (i = 0; i < 100; i++)
    {
        fprintf(f,"|%lf|%lf|%lf|\n",functions.G[i], functions.F[i], functions.Y[i]);
        fprintf(f, "_____________________________________________________________________\n");
    }
    fclose(f);
    f123 = fopen("/home/user/conclusion.txt","r");
    while (!feof(f123))
    {
        fscanf(f123, "%s", value);
        printf("%s\n", value);
